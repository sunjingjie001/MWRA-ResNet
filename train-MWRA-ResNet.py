...
from model_windows import MWRA_ResNet
...
def main():
	...
	check_point_path = 'E:/ResNet/ResNet_交叉熵多尺度_2_4_8_区域注意力改进/result__'+str(check_point)+'_start'
	...
	netResNet = MWRA_ResNet(device_num)
	...
	if use_tensorboard:
		...
		configure('E:/log对比/ResNet_交叉熵多尺度_2_4_8_区域注意力改进', flush_secs=5)
	...
	if check_point != 0:
		if torch.cuda.is_available():
			netResNet.load_state_dict(torch.load('E:/ResNet/ResNet_交叉熵多尺度_2_4_8_区域注意力改进/result__0_start/netResNet_epoch_' + str(check_point) + '_gpu.pth'))

		else :
			netResNet.load_state_dict(torch.load('E:/ResNet/ResNet_交叉熵多尺度_2_4_8_区域注意力改进/result__0_start/netResNet_epoch_' + str(check_point) + '_cpu.pth'))
